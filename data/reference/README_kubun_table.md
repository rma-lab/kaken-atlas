...
2026-08-28 生成、2026-09-23 に「内容の例」を現行版へ更新。生成: `uv run python scripts/build_kubun_table.py`（既定出力先=このディレクトリ）。
...
- **階層・コード・名称（日英）**: NII 公式マスタ
  ...
  2026-08-28 に公式マスタおよび実データ（KAKEN 採択課題23.9万件）と全数照合し、コード・名称・所属関係すべて正しいことを確認済み
  （**この全数照合は階層・コード・名称について**。「内容の例」は対象外）
- **小区分の「内容の例」（`sho_keywords` / `keywords`）**: `shinsa_kubun_r4.json` ＝ JSPS 審査区分表 別表2（令和4年3月9日決定、
  令和5年度審査から適用）。令和9年度公募ページ配布の PDF から抽出し、NII マスタと突合したもの（whet プロジェクト、2026-09-23。
  出所と照合記録はファイル内）。令和4年改正で「内容の例」が見直され、**99 小区分で 2018 年初版と語彙が異なる**
  （例: 01040 思想史関連に「イスラーム思想史」が追加、03070 博物館学関連は語彙が入れ替わり）。
- **2018 年初版の「内容の例」**（`sho_keywords_2018` / `keywords_2018`）: 手作りの `KubunTable.csv`
  （`~/Library/CloudStorage/Dropbox/研究IR/区分分類/BERT/`、Shift-JIS）。参考として残す。分析には現行版を使うこと
...
| sho_keywords | 小区分の「内容の例」（現行、令和4年改正版） |
| sho_keywords_2018 | 同、2018 年初版（参考） |
