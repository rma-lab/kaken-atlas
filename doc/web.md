...
- 裏側の点を隠すために不透明な球（半径 0.985）を置いている。点（半径≈1）との隙間が小さいため、縮小して視点が遠ざかると
  深度バッファの分解能が落ち、**同心の縞（z-fighting）**が出る。球の半径や点の散らしを固定で変えても、平行投影にしても解消しない。
  対策は**視距離の 2 乗に比例して球を小さくする**（隙間 0.015×(d/d0)²、下限 0.6。透視投影の深度誤差は距離の 2 乗に比例）。
- 点を不透明にすると gl-scatter3d の不透明経路が半透明の 3〜6 倍遅い。半透明にすると重なりの最上層が描画順に偏るので、
  乱数で組に分けて描画順を回転させる（§3）。
- 縮小時に球が小さくなるぶん、縁で裏側の点がわずかに見えるが、点も小さいので目立たない。
