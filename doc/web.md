...
- **2D の縦横比は `scaleanchor` を使わず自前で保つ**（v1.3 後の修正）。`scaleanchor` があると範囲変更のたびに Plotly が全再計算の経路に入り、
  点ごとの色配列では 1 回 190ms（外すと 30ms）。`equalScale(fit)` が初期表示（全体が収まる等スケール）と、範囲変更後の補正
  （x を正として y の幅を画素比に合わせる。ずれ 0.1% 未満なら何もしない）、自動範囲に戻ったときの決め直しを担う。
  フライ・トゥの着地範囲も画素比から y の半幅を決める。
...
- **なげなわ集計は自前実装**（v1.3 後）。Plotly の lasso/select は選択後に全トレースの `selectedpoints` 再計算を走らせ、点ごとの
  色配列では数秒（スマホでは固まって見える）。ツールバーの「囲って集計」で SVG オーバーレイに多角形を描き（pointer イベント、
...
- **既定はオフ**（2026-09-18 ユーザ決定）。切替はツールバーの「地名」ボタン（`placenamesButton`、点灯＝表示中）と「操作」のチェック
  （`setPlacenames()` で連動、`localStorage` の `ka-placenames`）。初回だけ `placenamesHint()` がボタンの下に案内を出す（`ka-pn-hint`、
  30 秒で消える）。2D はオンなら全体表示でも粗い階層を出す（`PN_ZOOM[0]=1.0`）。
...
- **リファクタリングの安全網**（2026-09-11）: `tests/web/artifacts.py`（points.bin・shards・manifest・sw.js の版のハッシュを記録・照合。
  Python 側の整理で 1 バイトも変わらないことを確かめる）と `tests/web/shots.js`（9 構図の基準スクリーンショットとの画素差分。
  同じ機械なら 0.000% で再現する）。整理はブランチで小さく刻み、各コミットで回帰試験・生成物・画素差分の 3 つを通す。
...
- 地図ページには脚注（出典・課題番号）を置かない（2026-09-20）。出典表示はトップページ・README・メタ情報・noscript で担う。
