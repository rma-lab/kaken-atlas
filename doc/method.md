...
将来の改良は 2 段階を想定している。(a) Ruri で峰の重心（山域の課題ベクトルの重み付き平均、正規化）を作り、候補語彙（著者キーワード）を
埋め込んで「その峰への近さ − 他の峰への近さの平均」で選ぶ（語彙全体から意味で選べる。語は作れない）。(b) LLM に山域の特徴語・
タイトル・概要を渡して命名する（日英同時。モデル・日付・プロンプトを記録し、生成物として保存。Ruri で峰の重心との近さを検算）。
どちらも JSON を差し替えるだけでサイトに載る。
