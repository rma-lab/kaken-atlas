...
§8.2 の連続色も同じ色相環に較正しているので同じ問題を持つ。地図では大区分の絞り込みで補える。改善案は色相環の順序を
保ったまま明度を周期的に振り、「色覚配慮」トグルで切り替えること。
...
- **色覚**：[§8](#8-配色意味順色相環と内容由来の連続色) の未対応事項。
- **研究者名・所属機関は載せない**方針（§1）。実務での必要は、利用者が手元データを持ち込むレイヤで応える（[roadmap.md](roadmap.md)）。
- 実装上の制約（Plotly、転送量、端末保存）は [web.md](web.md) を参照。

## 11. 種目の比較（補助分析）

「挑戦的研究（開拓・萌芽）はどこに位置するか」という問い（2026-09-11、Facebook のコメント）に答えるために作った 2 本。
種目名を変えれば他の種目群にも使える。

- `scripts/plot_category_contrast.py`：対象種目群とそれ以外の**シェア密度の差**を 2D 地図上に描く。§9 の偏差図と同じく
  |差| > 2σ の画素だけ着色（σ は両群の半分割ブートストラップを合成）。赤ピーク上位 3 箇所には周辺（半径 0.5）の対象課題の
  特徴語（周辺内件数 /（全体件数 + 20））を付ける。全変動距離で分布の違いの大きさも出す。
- `scripts/knn_dist_by_category.py`：768 次元で全課題の最近傍 k=15 件までのコサイン距離を計算し（`data/processed/knn_dist.parquet`
  に保存）、種目群ごと・大区分ごとに比べる。近傍のうち同じ種目群が占める割合（同類性）も出す。

**挑戦的研究について分かったこと**（2026-09-11）：
- 分布は基盤研究と大きくは変わらない（全変動距離 0.24）。相対的に多いのは物理・材料（表面プラズモン、熱電材料）と生物計測
  （生物発光、1 分子計測）、生殖・発生生物学で、周辺の 12〜13% を占める（全体平均 4.9%）。少ないのは文学と看護（1% 台）。
- **周りの課題との距離は長くない**。最近傍 15 件の平均距離の中央値は挑戦的 0.122、基盤(B)(C) 0.124、若手 0.121 で、大区分別でも
  並ぶ（B〈数物系〉だけ挑戦的がやや長い）。種目の差より分野の差（I 0.116、A 0.130）のほうがはるかに大きい。
  最も密な中心部を避ける弱い傾向（密度分位の上位 10% にいる割合 6.1%、対照 10%）はある。
- 挑戦的研究の近傍に挑戦的研究が占める割合は 10.6% で全体比率の 2 倍。内容の近さか申請書の文体かは未分離。
- 半透明の小さな点で描くと、20 万点では密集地が飽和して粗密と縁取りが強調され、1 万点では一様に見える。**件数を揃えないと
  見た目の比較はできない**（無作為 1 万件の対照を並べて確認）。

次の分析候補（[roadmap.md](roadmap.md) §1）：同じ研究者の基盤研究からの飛距離、位置でマッチングした対照との近傍の分野多様性、
挑戦的研究が多い場所で後年に基盤研究が増えるかのリードラグ。

## 12. 地名：峰と山域、キーワード、LLM による抽象化（2026-09-17〜18）

地図に地名を置くとき、公式区分名を使うと「人が引いた線」が第一印象になる。公開版の地名は**課題の内容だけから**作る。
まず密度の峰ごとに「山域」を切り、山域ごとの特徴語（キーワード地名）を基準として求め、表示にはそれを LLM で一段抽象化した名前を使う
（`scripts/compute_placenames.py`。公式区分の名前は将来、比較用の切替層として重ねる）。

1. **密度の地形**：2D 座標の格子ヒストグラム（長辺 640 画素）にガウス平滑（σ は座標単位）をかける。
2. **峰と山域**：密度の局所最大（最大密度の 3% 以上、σ 以上離れたもの）を峰とし、**密度を上下反転した地形に分水嶺法**を掛けて
   峰ごとの領域を得る。反転しているので窪みが密度の山、境界が密度の谷（地図の空白）になる。この領域を「山域」（峰から裾野まで、
   谷で隣と分かれるひとまとまり）と呼ぶ。「流域」は誤り（流域の中心は谷）。峰をつなぐ尾根の上の課題は鞍部で隣の山域に分かれるので、
   鞍部の課題は架橋的な研究の候補になる（R9）。
3. **階層**：σ=0.3（峰 42）と σ=0.15（峰 129）の 2 段。ズームに応じて切り替える。150 件未満の山域には地名を付けない。
4. **語の選択**：山域内のキーワードを集中度 n_in /（n_all + 30）で採点する（凡例の 12 方位と同じ。頻度で選ぶと「開発」「解析」が
   上位に来る）。これに n_in^0.5 を掛けて狭すぎる専門語より広く使われる語を少し優先し、**各課題は「その位置の密度 ÷ 峰の密度」で重み付け**
   する（山頂の課題は 1、鞍部は小さい。縁の課題は隣の峰にもまたがるため）。除外語（汎用語）と、同じ地名の中の表記ゆれ
   （一方が他方を含む、文字 2-gram の重なり 50% 以上。がん／癌は同一視）を除いて上位 2 語。
5. **検算**：静的図（`scripts/plot_placenames.py`）で既知の場所を確かめる。眼科の島＝「緑内障・加齢黄斑変性」、量子＝「暗黒物質・ニュートリノ」、
   生成 AI の密集地＝「教師教育・プログラミング教育」「対話システム・ヒューマンロボットインタラクション」など。重み付けなし（ALPHA=0）では
   2.7 万件の医学の山に「骨芽細胞」のような狭い語が付いたので、上の重みを採用した。

**球面版**（`scripts/compute_placenames_sphere.py`、2026-09-17）。球面の配置は 2D とは別の UMAP なので、地名も球面上で求め直す。
緯度経度の格子は極で歪むため、ほぼ均等なフィボナッチ格子（15 万点、間隔 0.52°）に課題を数え、格子点どうしのガウス重み
（大円距離、σ0=0.03 rad）で平滑化する。粗い階層は同じ平滑化を 4 回重ねる（幅は σ0×√n）。峰は大円距離 σ 以内で最大の格子点、
山域は峰を種に密度の高い順に隣接格子点へ広げる分水嶺。2D と違う点が 3 つある。
(1) 閾値の基準を最大密度でなく**上位 1% 点**にした。球面の配置（min_dist=0）には中心が他の峰の約 10 倍に突出した塊が 2 つあり、
最大値基準では周辺の峰が落ちる。(2) その 2 つの塊（物性・材料、代謝・循環器）は 1 万件が半径 2° ほどに潰れた核と、峰から 15° 以上に
広がる裾野からなり、内部に小さな峰を持たない。裾野はまばらな周辺（人文系の一部を含む）まで及ぶので、**件数が閾値を超える山域は位置
（球面上の k-means）で区画に分け、区画ごとに名付ける**。(3) 粗い階層は既定の表示で出るため、地名を付ける山域を 1,000 件以上に限る
（まばらな側の小さな島の名前が大きな領域と同格に見えないように）。結果は 45 か所と 159 か所。

**LLM による抽象化**（`scripts/name_places_llm.py`、2026-09-18、サイトの既定）。キーワード地名は著者キーワードの中から選ぶので、
上位概念（「がんの免疫療法」）は語彙になく選べない。そこで山域ごとに課題の本文（埋め込みに使ったタイトル＋キーワード＋概要）を
LLM（Claude Opus 5）に読ませ、領域全体を包含する 4〜12 文字の名詞句を付ける。
- 抽出: 山域の課題から、中心重み（その位置の密度 ÷ 峰の密度。山の形に沿った不定形の重みで、峰からの距離ではない）を確率にして
  k 件を無作為抽出（粗い階層 25、細かい階層 15。乱数の種は固定）。中心の課題が多く、裾野の課題が少し混じる。
- 指示: 階層（粗い＝地図全体を約 40 か所に分けた 1 つ／細かい＝親「…」の一部）と件数を伝え、個々の専門用語を並べない、公式区分名を
  そのまま使わない、隣と区別がつく程度に具体的、と条件を付ける。構造化出力で日本語名・英語名・根拠・混在フラグ（2 つ以上の主題が
  混じる。その場合は最も多い主題の名前）を受け取る。文字種（日本語とギリシャ文字）と長さを検査し、外れたら言い直させる。
- 結果: 2D は 42＋126 か所、球面は 45＋159 か所。混在判定は 2D の粗い階層で 8/42、球面では 19/45（球面の巨大な山域の裾野は
  まばらな周辺を巻き込むため）。費用は約 20 ドル。出力はモデル・日付・プロンプト版・抽出した課題番号・使用トークンとともに
  `data/processed/placenames_llm_<map>_L<level>.json` に保存し、サイトはそれを読むだけ（`PLACENAMES_SOURCE=keywords` で戻せる）。
- 再現性の位置づけ: LLM の出力は完全には固定できないので、生成物を保存して版を記録する。キーワード地名を「基準」、LLM の名前を
  「読みやすい表記」と位置づける。未実装: Ruri で名前を埋め込み峰の重心との近さを検算すること、Ruri で候補語彙から選ぶ中間案。
