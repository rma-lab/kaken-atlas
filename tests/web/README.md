...
| 10_placenames | 地名（既定オフ）。初回は案内が出る、ツールバーの「地名」でオン（2D は全体表示でも粗い階層、4 倍で細かい階層。重なりなし・描画領域内）、移動後も描かれる、設定が残り案内は 2 回目に出ない、チェックでオフ。球面: ボタンでオン、回すと入れ替わる、近づくと細かい階層。3D には層もボタンもない |
