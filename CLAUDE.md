...
- **研究者名・所属機関は地図サイトに載せない。検索・絞り込み・彩色の対象にもしない。** 配置は研究内容（採択時概要）のみ、
  色は公式区分のみ。名前や機関の先入観なしに研究内容で学術を俯瞰してもらうため。個人・機関の情報は KAKEN へのリンク先に委ねる。
...
## バージョン管理（公開サイト）
- `CHANGELOG.md` に変更を記録し、区切りで **git タグ `vX.Y` ＋ GitHub Release** を切る（2026-09-08 開始、現行 v1.2）。
