...
> 開発者向けの技術ノート（公開）は 3 部: `doc/method.md`（方法と可視化の設計）、`doc/web.md`（サイトの実装と運用）、
> `doc/roadmap.md`（R9 以降の設計メモ）。**方針を変えたら該当する方を更新する**。
...
## サイトの方針（2026-09-09 決定）
- **研究者名・所属機関は地図サイトに載せない。検索・絞り込み・彩色の対象にもしない。** 配置も色も研究内容（採択時概要）のみ
  （色は 2026-09-11 に大区分 11 色から内容由来の連続色へ変更。公式区分は絞り込みの層としてのみ残す）。
  名前や機関の先入観なしに研究内容で学術を俯瞰してもらうため。個人・機関の情報は KAKEN へのリンク先に委ねる。
  「研究者・機関の層」のような可視化はこの方針と衝突するので原則やらない（自分の文章を地図に置く機能は両立）。
- **公開版は「本来の思想」に沿って作る（2026-09-11）**: 地図が見せるのは人が線を引く前の地形。公式区分名の地名や機関別の絞り込みなど
  人為的な区画を強調する機能・実務者向けの強化は上流の優先事項にしない（フォークや共同開発者に委ねる）。
  地名を付けるなら課題のキーワードなどデータ由来のものを既定にし、公式区分は比較のために切替で重ねる層に留める。
...
- 大区分11色は**意味順色相環**（重心距離の最短巡回路→OKLCH等間隔。導出は `scripts/plot_map_dai.py` コメント）。
  静的図・アイコン・連続色の色相較正に使う。
- **地図サイトの点の色は内容由来の連続色（弾性リング、2026-09-11 決定）**: `scripts/plot_map_textcolor.py d` でリングを学習
  （`data/interim/elastic_ring_nodes.npz`）→ `scripts/compute_textcolor.py` で色表 `data/processed/textcolor_d.parquet` と
  凡例 `textcolor_legend.json` を生成 → `build_web_map.py`（既定 `POINT_COLOR=text`）が points.bin に同梱。
  埋め込みを更新したらリングから再学習する。彩度の決め方は compute_textcolor.py のコメント参照。
...
## バージョン管理（公開サイト）
- `CHANGELOG.md` に変更を記録し、区切りで **git タグ `vX.Y` ＋ GitHub Release** を切る（2026-09-08 開始、現行 v1.7）。
